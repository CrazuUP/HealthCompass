const HealthApp = {
  ...
  fillProfileForm() {
    ...
    if (birthYearEl) birthYearEl.value = user.birthYear || '';

    const genderEl = document.getElementById('gender');
    if (genderEl) genderEl.value = user.gender || '';

    // Обрабатываем группу крови (может быть в формате "A+" или "A" и отдельно резус)
    const bloodTypeEl = document.getElementById('bloodType');
    const rhFactorEl = document.getElementById('rhFactor');
    if (user.bloodType) {
      if (user.bloodType.length > 1 && (user.bloodType.endsWith('+') || user.bloodType.endsWith('-'))) {
        // Формат "A+" или "A-"
        const bloodType = user.bloodType.slice(0, -1);
        const rhFactor = user.bloodType.slice(-1);
        if (bloodTypeEl) bloodTypeEl.value = bloodType;
        if (rhFactorEl) rhFactorEl.value = rhFactor;
      } else {
        // Только группа крови без резуса
        if (bloodTypeEl) bloodTypeEl.value = user.bloodType;
      }
    }
    ...
    if (heightEl) heightEl.value = user.height || '';

    const emergencyContactEl = document.getElementById('emergencyContact');
    if (emergencyContactEl) emergencyContactEl.value = user.emergencyContact || '';

    // Заполняем дополнительные поля здоровья
    if (user.healthConditions) {
      const allergiesEl = document.getElementById('allergies');
      if (allergiesEl) allergiesEl.value = user.healthConditions.allergies || '';

      const visionEl = document.getElementById('vision');
      if (visionEl) visionEl.value = user.healthConditions.vision || '';

      const workTypeEl = document.getElementById('workType');
      if (workTypeEl) workTypeEl.value = user.healthConditions.workType || '';

      const medicalHistoryEl = document.getElementById('medicalHistory');
      if (medicalHistoryEl) medicalHistoryEl.value = user.healthConditions.medicalHistory || '';

      const currentConditionsEl = document.getElementById('currentConditions');
      if (currentConditionsEl) currentConditionsEl.value = user.healthConditions.currentConditions || '';
    }
  },
  ...
  setupEventListeners() {
    ...
    window.clearProfile = () => this.clearProfile();
    ...
    window.showEmergencyAlert = () => this.showEmergencyAlert();
  ...
  // Получить имя пользователя строго по имени
  getUserFirstName() {
    if (!this.currentUser || !this.currentUser.fullName) {
      return '';
    }
    const nameParts = this.currentUser.fullName.trim().split(/\s+/);
    // Имя - это второе слово (индекс 1), если есть, иначе первое
  ...
  // Очистка профиля
  clearProfile() {
    if (confirm('Очистить профиль? Все данные профиля будут удалены, и вы сможете создать новый профиль.')) {
      localStorage.removeItem('healthCompassUser');
      this.currentUser = null;
      const form = document.getElementById('profileForm');
      if (form) form.reset();
      this.showNotification('Профиль очищен. Заполните форму для создания нового профиля.');
      // Показываем экран профиля, если мы не на нём
